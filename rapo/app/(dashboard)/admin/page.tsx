...
const ROLES = [
  ...
  { value: "Data Controller", label: "Data Owner" },
  { value: "DPO", label: "DPO" },
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await fetch(`${API_URL}/api/users`);
        const data = await res.json();
        console.log("RAW:", JSON.stringify(data[0]));
        if (!res.ok) throw new Error(data.error || "Failed to fetch users");

        // mapping relationship of DB (nested relationship)
                    ...
                    {menuIndex === index && (
                      ...
                      >
                        <button
                          ...
                          onClick={() => { setSelectedUser({ ...user, _index: index }); setActionModal("edit"); setMenuIndex(null); }}
                        >Edit</button>
                        <button
                          className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100 text-red-500"
                          onClick={() => { setSelectedUser({ ...user, _index: index }); setActionModal("delete"); setMenuIndex(null); }}
                        >Delete</button>
                        <button
                          className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100"
                          onClick={() => { setSelectedUser({ ...user, _index: index }); setActionModal("role"); setMenuIndex(null); }}
                        >Change Role</button>
                      </div>
                    )}
                  </div>
      ...
      {actionModal && selectedUser && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999]">
          <div className="bg-white rounded-xl w-[420px] p-6 shadow-2xl">
              ...
              <>
                ...
                <div className="flex justify-end gap-2 mt-5">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#203690] text-white rounded-lg text-sm"
                  >Save</button>
                </div>
              </>
            )}
            ...
            {actionModal === "role" && (
              <>
                <h2 className="text-base font-semibold mb-4">Change Role</h2>

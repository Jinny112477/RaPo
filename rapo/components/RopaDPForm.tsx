...
function newSub(i: number): SubActivity {
  return {
    ...
    transferAffiliateCompany: '', transferMethod: '', transferStandard: '', transferException28: '',
    storageType: [], storageMethod: '', retentionPeriod: '', accessRights: '', deletionMethod: '',
  };
}
      ...
      {open && (
        <div className="p-5 space-y-5 bg-white">
          ...
          <Field label="ข้อมูลส่วนบุคคลที่จัดเก็บ" required>
            <MultiCheck options={PERSONAL_DATA_ITEMS} selected={sub.personalDataItems} onChange={v => set('personalDataItems', v)} cols={3} />
          </Field>
...
export default function RopaDPForm({ activityId, onSubmit, onSaveDraft }: RopaFormProps) {
  ...
  const handleSaveDraft = () => {
    onSaveDraft?.({ processorName, mainActivity });
    setDraftSaved(true);
      ...
      {step === 4 && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700 leading-relaxed flex gap-2">
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5 text-amber-500" />
              <span>
                <span className="font-semibold block mb-0.5">หมายเหตุ</span>
                เมื่อส่งแล้ว สถานะจะเปลี่ยนเป็น &ldquo;รอการตรวจสอบ (REVIEW)&rdquo; และ DPO จะต้องอนุมัติก่อนจึงจะมีสถานะ Active
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {step < TOTAL ? (
            <button type="button" onClick={() => canNext() && setStep(s => s + 1)}
              disabled={!canNext()}
              className="inline-flex items-center gap-1.5 px-5 py-2 text-sm font-semibold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
              ถัดไป
